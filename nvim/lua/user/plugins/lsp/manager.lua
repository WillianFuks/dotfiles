...
local function build_mason_config(server_name, pkg_name)
    if pkg_name == nil then return {} end
    local found, mason_config = pcall(
        require,
        'mason-lspconfig.server_configurations.' .. server_name
    )
    if not found then
        return {}
    end
...
local function default_on_attach(client, bufnr)
    set_codelens_refresh_autocmds(client)
...
function M.setup(server_name)
    if (is_server_active(server_name) or
        client_is_configured(server_name)) then return end
    local pkg_name = require('mason-lspconfig.mappings.server').lspconfig_to_package[server_name]
    local registry = require 'mason-registry'
    if not registry.is_installed(pkg_name) then
        vim.notify_once(
            string.format('Installation in progress for: [%q]', server_name),
            vim.log.levels.INFO
        )
